...
/datum/manor/proc/get_manor_size(mob/living/carbon/human/owner)
	if(owner)
		/*if(is_foreign_estate(owner))
			...
			return "small"*/
		if(HAS_TRAIT(owner,TRAIT_MANORHOLDERBIG))
			return "big"
		if(HAS_TRAIT(owner,TRAIT_MANORHOLDERMEDIUM))
			return "medium"
	return "small"
...
/datum/manor/proc/send_foreign_estate_income_mail(mob/living/carbon/human/owner, total_profit_money, estate_levy, import_tariff)
	...
	if(!SSroguemachine.hermailermaster)
		if(owner.client)
			to_chat(owner, span_warning("Your foreign income can not be delivered - HERMES is unavailable."))
		return
	...
	P.info += "Sending you profit recieved from goods produced by your peasants and workers during the last dae."
	P.info += "\nPure profit: [total_profit_money] mammon."
	if(estate_levy)
		P.info += "\nEstate levy: [estate_levy] mammon."
	if(import_tariff)
		P.info += "\nImport tariffs: [import_tariff] mammon."
	P.update_icon()
...
/datum/manor/proc/produce_resources(mob/living/carbon/human/owner, is_dawn = FALSE, is_dusk = FALSE)
	...
	if(total_profit_money)
		if(patron == /datum/patron/inhumen/matthios)
			message += "your comrades willingly sent you [total_profit_money] mammon"
		else
			message += "Pure profit is [total_profit_money] mammon"
		if(estate_levy)
			message += ", with [estate_levy] mammon of estate levy"
		if(import_tariff)
			message += (estate_levy ? " and" : " ") + "[import_tariff] mammon of import tariffs"
		if(is_foreign)
			message += ". The profit is sent to you via HERMES"
		message += "."
	else
		if(patron == /datum/patron/inhumen/baotha)
			message += "your strward is seemingly too occupied with debauchery and your profit was not delivered."
		else if(patron == /datum/patron/inhumen/matthios)
			message += "your comrades decided not to send you mammon this time."
		else
			message += "there is no profit from manor."
	if(owner.client)
...
/obj/structure/roguemachine/mail/proc/open_manor_panel(mob/user)
	...
	if(!manor)
		qdel(panel)
		to_chat(H, span_warning("This character has no available manors yet."))
		return TRUE
